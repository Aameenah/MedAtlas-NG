  ...
  if (step === 'success') {
    return (
      <div className="max-w-md mx-auto mt-10 px-4 animate-fade-in-up">
        <div className="bg-white rounded-3xl shadow-xl p-8 text-center border border-purple-100">
          ...
          {step === 'datetime' ? (
            <div className="animate-fade-in-up">
               <h3 className="font-bold text-slate-900 mb-4 flex items-center">
                 <Calendar className="w-5 h-5 text-purple-600 mr-2" /> Select Date
               </h3>
               <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar mb-8">
                 {dates.map((d, i) => (
                   ...
                   >
                     ...
                     <span className="text-2xl font-bold">{d.date.split(' ')[0]}</span>
                   </button>
                 ))}
               </div>

               <h3 className="font-bold text-slate-900 mb-4 flex items-center">
                 <Clock className="w-5 h-5 text-purple-600 mr-2" /> Select Time
               </h3>
               <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-10">
                 {timeSlots.map((time) => (
                   <button
                     ...
                     className={`py-3 px-2 rounded-xl text-sm font-bold transition-colors border-2 ${
                        selectedTime === time
            ...
            <div className="animate-fade-in-up">
                <div className="bg-slate-50 p-6 rounded-2xl mb-8 border border-slate-100">
                    ...
                    <div className="border-t border-slate-200 my-4 pt-4 space-y-2">
                        <div className="flex justify-between items-center text-slate-500">
                            <span>Consultation Cost <span className="text-[10px] uppercase bg-slate-200 px-1 rounded ml-1">Pay at Facility</span></span>
                            <span className="font-medium">₦{service.price.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center text-purple-700">
                            <span className="font-bold">Booking Fee <span className="text-[10px] uppercase bg-purple-100 px-1 rounded ml-1">Pay Now</span></span>
                ...
                <div className="mb-8">
                    <h3 className="font-bold text-slate-900 mb-4 flex items-center">
                        <CreditCard className="w-5 h-5 text-purple-600 mr-2" /> Payment Method
                    </h3>
                    <div className="p-4 border-2 border-purple-600 bg-purple-50 rounded-xl flex items-center justify-between cursor-pointer">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-6 bg-slate-800 rounded flex items-center justify-center text-white text-[8px] font-bold">
                                CARD
                            </div>
                            <span className="font-bold text-slate-900">Pay with Card / USSD</span>
                        </div>
                        <div className="w-5 h-5 rounded-full border-[5px] border-purple-600"></div>
                    </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-green-200 flex items-center justify-center gap-2"
                >
                   {isProcessing ? (
