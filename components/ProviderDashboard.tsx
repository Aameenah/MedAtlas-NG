...
export const ProviderDashboard: React.FC<ProviderDashboardProps> = ({ appointments = [] }) => {
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-10">
      <div className="bg-white border-b border-purple-100 px-6 py-4 mb-6 sticky top-16 z-30">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Reception Desk</h1>
            <p className="text-sm text-slate-500">Kwara Advanced Medical Center</p>
          </div>
          <div className="flex items-center gap-3">
             ...
             <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=200" className="w-10 h-10 rounded-full border-2 border-purple-200" alt="Profile" />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-purple-100 flex items-center justify-between">
                <div>
                    ...
                    <h3 className="text-3xl font-bold text-purple-900 mt-1">{todayCount}</h3>
                </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-purple-100 flex items-center justify-between">
                <div>
                    <p className="text-slate-500 text-sm font-medium">Pending Confirmation</p>
             ...
             <div className="bg-white p-6 rounded-2xl shadow-sm border border-purple-100 flex items-center justify-between">
                <div>
                    <p className="text-slate-500 text-sm font-medium">Total Patients (Week)</p>
                    <h3 className="text-3xl font-bold text-green-600 mt-1">84</h3>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-purple-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-purple-50/50">
                    ...
                    <span className="text-sm bg-purple-100 text-purple-700 px-3 py-1 rounded-full font-medium">Today</span>
                </div>
                    ...
                    {sortedAppointments.map((apt) => (
                        <div key={apt.id} className={`p-4 hover:bg-purple-50/30 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${apt.status === 'pending' ? 'bg-orange-50/30' : ''}`}>
                            <div className="flex items-center gap-4">
                                <div className="font-mono text-slate-500 font-semibold w-20 text-center bg-slate-50 rounded-lg py-2">
                                    {apt.time.split(' ')[0]} <span className="text-xs">{apt.time.split(' ')[1]}</span>
                                </div>
                                <div>
                                    <h4 className="font-bold text-slate-900">{apt.patientName}</h4>
                                    <p className="text-sm text-slate-500">{apt.serviceName} • {apt.doctorName}</p>
                                </div>
                            </div>
